...
our @EXPORT =
  qw/new root_dir psgi build_app _router _connect get post filter wrap_filter/;
...
sub build_app {
    ...
    sub {
        ...
        my $match = try {
            local $env->{PATH_INFO} =
              Encode::decode_utf8( $env->{PATH_INFO}, 1 );
            $router->match( $env );
        }
        catch {
            warn $_;
...
sub wrap_filter {
    ...
    Carp::croak sprintf( "Filter:%s is not exists", $filter )
